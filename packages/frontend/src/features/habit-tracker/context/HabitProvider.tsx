...
import { HabitContext, type Habit, type HabitEntry, type HabitContextType, type CreateHabitRequest, type UpdateHabitRequest } from './habit-context';
import { habitService, type Habit as BaseHabit } from '../../../services/habitService';
...
const HabitProvider = ({ children }: { children: ReactNode }) => {
  ...
  const habits: Habit[] = useMemo(() => {
    ...
    return baseHabits.map(baseHabit => {
      const todayEntry = habitEntries.find(entry => 
        entry.habitId === baseHabit.id && entry.date === today
      );
  ...
  const refreshHabitEntries = useCallback(async (startDate?: string, endDate?: string) => {
    if (!user) return;
    ...
    try {
      ...
      const now = new Date();
      const defaultStartDate = startDate || getLocalDateString(new Date(now.getFullYear(), now.getMonth(), 1));
      const defaultEndDate = endDate || getLocalDateString(new Date(now.getFullYear(), now.getMonth() + 1, 0));
      
      const fetchedEntries = await habitService.getHabitEntries(defaultStartDate, defaultEndDate);
      setHabitEntries(fetchedEntries);
    } catch (error) {
      console.warn('Cloud Functions not available, using empty entries:', error);
      // Fallback to empty state when Cloud Functions are not available
      setHabitEntries([]);
      setError(null); // Don't show error for missing backend
    } finally {
  ...
  }, [user, refreshHabits, refreshHabitEntries]);
  ...
  const updateHabit = useCallback(async (habitId: string, updates: UpdateHabitRequest) => {
    try {
      ...
      setBaseHabits(prev => prev.map(habit => habit.id === habitId ? updatedHabit : habit));
    } catch (error) {
  ...
  const reorderHabits = useCallback(async (habitIds: string[]) => {
    try {
      ...
      const reorderedHabits = habitIds.map(id => baseHabits.find(h => h.id === id)!).filter(Boolean);
  ...
  const updateHabitEntry = useCallback(async (
    habitId: string, 
    date: string, 
    count: number, 
    completed: boolean, 
    ...
    try {
      ...
      // Refresh habits to get updated analytics
      refreshHabits();
    } catch (error) {
      handleError(error, 'Updating habit entry');
      throw error;
    }
  }, [handleError, refreshHabits]);

  const deleteHabitEntry = useCallback(async (habitId: string, date: string) => {
    try {
      setError(null);
      await habitService.deleteHabitEntry(habitId, date);
      setHabitEntries(prev => prev.filter(entry => !(entry.habitId === habitId && entry.date === date)));
  ...
  const getHabitEntriesForDate = useCallback((date: string): HabitEntry[] => {
    return habitEntries.filter(entry => entry.date === date);
  }, [habitEntries]);

  const getHabitEntriesForHabit = useCallback((habitId: string): HabitEntry[] => {
    return habitEntries.filter(entry => entry.habitId === habitId);
  }, [habitEntries]);

  // Legacy compatibility methods
  const addHabit = useCallback(async (habitData: Omit<CreateHabitRequest, 'days'> & { days?: number[] }) => {
  ...
  const incrementHabit = useCallback(async (habitId: string, date: Date) => {
    const dateString = getLocalDateString(date);
    const currentEntry = habitEntries.find(entry => entry.habitId === habitId && entry.date === dateString);
  ...
  const decrementHabit = useCallback(async (habitId: string, date: Date) => {
    const dateString = getLocalDateString(date);
    const currentEntry = habitEntries.find(entry => entry.habitId === habitId && entry.date === dateString);
    const currentCount = currentEntry?.count || 0;
    const habit = getHabitById(habitId);
  ...
  const legacyReorderHabits = useCallback(async (activeId: string, overId: string) => {

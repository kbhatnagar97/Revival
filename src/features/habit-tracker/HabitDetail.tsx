  ...
  while (true) {
    ...
    if (scheduledDays.includes(dayOfWeek)) {
      const wasCompleted = completedDates.some(
        (d) => getLocalDateString(d) === dateString
      );
      if (wasCompleted) {
        currentStreak++;
      } else {
    ...
    if (completedDates.length > 0) {
      const timeSinceLastCompletion =
        new Date().getTime() - completedDates[0].getTime();
      const daysSinceLastCompletion =
        timeSinceLastCompletion / (1000 * 3600 * 24);
...
const calculateCompletionRate = (
  ...
  const completedOnScheduledDays = Object.entries(completionData).filter(
    ([date, count]) => {
      const d = new Date(date + 'T00:00:00');
      return (
        d >= firstLogDate && scheduledDays.includes(d.getDay()) && count >= goal
      );
    }
  ).length;
...
const calculateSurplusRate = (
  completionData: { [date: string]: number } = {},
  goal: number
): number => {
  const completedEntries = Object.values(completionData).filter(
    (count) => count >= goal
  );
  if (completedEntries.length === 0) return 0;
  const surplusEntries = completedEntries.filter(
    (count) => count > goal
  ).length;
...
const HabitDetail = () => {
  ...
  const chartConfig = useMemo(() => {
    ...
    switch (timeRange) {
      ...
      case 'all': {
        ...
        startDate =
          allDates.length > 0 ? new Date(allDates[0] + 'T00:00:00') : today;
        break;
      }
    }
    ...
    for (let d = new Date(startDate); d <= today; d.setDate(d.getDate() + 1)) {
      const labelFormat: Intl.DateTimeFormatOptions =
        timeRange === 'all' &&
        today.getTime() - startDate.getTime() > 90 * 24 * 60 * 60 * 1000
  ...
  if (!mainHabit) {
    return (
      <div className='habit-detail--not-found'>
        ...
        <button onClick={() => navigate('/habit-tracker')} className='back-btn'>
          {' '}
          <FaIcons.FaArrowLeft /> All Habits{' '}
        </button>
      </div>
    );
  }
  ...
  const debtPercentage =
    mainHabit.goal > 0 ? Math.round((liveDebt / mainHabit.goal) * 100) : 0;
  const surplusPercentage =
    mainHabit.goal > 0 ? Math.round((liveSurplus / mainHabit.goal) * 100) : 0;
  ...
  const selectedDayHabit = habitsForSelectedDate.find(
    (h) => h.id === mainHabit.id
  );
  const selectedDayProgress =
    selectedDayHabit && mainHabit.goal > 0
        ...
        <div className='detail-header'>
          ...
          <div className='habit-title'>
            <div
              ...
              style={{ backgroundColor: mainHabit.color }}
            >
              {' '}
              {(FaIcons as any)[mainHabit.icon]()}{' '}
            </div>
            <h1>{mainHabit.name}</h1>
            <button className='detail-edit-btn' onClick={handleOpenEditModal}>
              {' '}
              <FaIcons.FaPen /> Edit{' '}
            </button>
          </div>
          ...
          <div className='detail-main-content'>
            <div className='stats-container'>
              ...
              <div className='stats-grid'>
                ...
                <div className='stat-card'>
                  <div className='stat-card__icon icon--neutral'>
                    <FiCheckSquare />
                  </div>
                  <div className='stat-card__content'>
                ...
                <div className='stat-card'>
                  ...
                  <div className='stat-card__content'>
                    <div className='stat-value stat-value--surplus'>
                      {surplusPercentage}%
                    </div>
                    <div className='stat-label'>
                      Surplus
                ...
                <div className='stat-card'>
                  <div className='stat-card__icon icon--red'>
                    <FiTrendingDown />
                  </div>
                  <div className='stat-card__content'>
                    <div className='stat-value stat-value--debt'>
                      {debtPercentage}%
                    </div>
                    <div className='stat-label'>
                      Debt
            ...
            <div className='consistency-container'>
              ...
              <div className='consistency-cluster'>
                <div className='consistency-pod'>
                  ...
                  <ConsistencyRing
                    percentage={allTimeConsistency}
                    color={mainHabit.color}
                    lightenedColor={lightenColor(mainHabit.color, 40)}
                  />
                </div>
                <div className='consistency-pod'>
                  <h4 className='pod-title'>
                ...
                <div className='consistency-pod'>
                  ...
                  <ConsistencyRing
                    percentage={selectedDayProgress}
                    color={mainHabit.color}
                    lightenedColor={lightenColor(mainHabit.color, 40)}
                  />
                </div>

...
const HabitCard: React.FC<HabitCardProps> = ({ habit, onEdit, selectedDate, isSortable, isEditable = true }) => {
  ...
  const progressPercentage = habit.goal > 0 ? (Math.min(habit.count, habit.goal) / habit.goal) * 100 : 0;
      ...
      >
        <div className="card-header">
          ...
          <div className="habit-actions">
            {isEditable && (
              <button className="edit-btn" onClick={handleEditClick} aria-label={`Edit ${habit.name}`}>
                <FaIcons.FaPen />
              </button>
            )}
            ...
            <button className="action-btn decrement-btn" onClick={handleDecrementClick} disabled={habit.count === 0} aria-label={`Decrement ${habit.name}`}>
              <FaIcons.FaMinus />
            </button>
            <button className="action-btn increment-btn" onClick={handleIncrementClick} aria-label={`Increment ${habit.name}`}>
              <FaIcons.FaPlus />
            </button>
          </div>
        </div>
